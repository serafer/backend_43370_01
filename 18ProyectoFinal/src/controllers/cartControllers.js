  ...
  try {
    ...
    if (cart === "owner")
      return httpResponse.Unauthorized(
        res,
        "Role Premium: cannot add your own product"
      );
    else return httpResponse.Ok(res, cart);
  } catch (error) {
    next(error.message);
...
export const updateQuantityInCart = async (req, res, next) => {
  try {
    ...
    const cart = await service.updateQuantityInCartService(
      id,
      productId,
      quantity
    );
...
export const generateTicketViews = async (req, res, next) => {
  try {
    const user = await getUserByID(req.user);
    if (!user) {
      return res.redirect("/error-alert");
    }
...
export const getAllTickets = async (req, res, next) => {
  try {
    ...
    const tickets = await service.getAllTicketsServices(purchaser);
